...
public:
    ...
    explicit TUserViewDialog(Core::UserInfo_Ptr &inUserInfo, QWidget *inParent = nullptr);
    ~TUserViewDialog();
...
private:
    ...
    Core::UserInfo_Ptr fUserInfo ; // Указател на объект информации о пользователе

    void initFont(); // Метод инициализирует стиль текста
    void initText(); // Метод инициализирует надписи

signals:
    void sig_Result(const Core::UserInfo_Ptr inUserInfo, qint32 inResult);
