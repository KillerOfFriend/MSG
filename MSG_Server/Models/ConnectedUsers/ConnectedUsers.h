...
class TConnectedUsersModel : public QAbstractTableModel, public std::map<QTcpSocket*, Core::TUserAccount>
{
public:
    ...
    //-- Методы контейнера
    std::pair<std::map<QTcpSocket*, Core::TUserAccount>::iterator, bool> insert(std::pair<QTcpSocket*, Core::TUserAccount> inItem);
    void erase(std::map<QTcpSocket*, Core::TUserAccount>::iterator inIt);
    void clear();
